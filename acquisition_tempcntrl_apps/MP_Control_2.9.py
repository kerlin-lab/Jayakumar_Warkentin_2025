...
def mod_phase(phase_val):
    global abort_flag
    if abort_flag:
        raise InterruptedError("Aborted before setting phase")
    cmd = f":w31={phase_val}.\n"
    try:
...
def abort_DDS():
    try:
        ...
        print("Error during abort_DDS:", e)
...
def abort():
    global serial_device, abort_flag
...
try:
    root.iconbitmap(r"C:\Users\EOD\MP_icon.ico")
except:
    pass
...
# Baud rate
tk.Label(root, text="Baud Rate (bits/s):").pack()
...
# Drive Voltage
tk.Label(root, text="Drive Voltage (V, max=12 V):").pack()
...
tk.Button(button_frame, text="Start", command=start_thread, bg="#216D4A", fg="white").pack(side=tk.LEFT, padx=5)
pause_button = tk.Button(button_frame, text="Pause", command=pause_resume)
pause_button.pack(side=tk.LEFT, padx=5)
tk.Button(button_frame, text="Abort", command=abort, bg="#E64141", fg="white").pack(side=tk.LEFT, padx=5)
